         ...
         {/* Heading */}
             ...
             <div className=" grid grid-cols-3 gap-[50px] mt-10 w-[1200px] mx-auto  "> 
             {
                         storedata.map((item,index) => (
                           // card div
                           <div className=' '>
                            <div key={index} className=' bg-white w-[300px] hover:border-none hover:shadow-lg hover:shadow-white hover:scale-110 transition-all duration-500 border-2 border-slate-500 p-2 rounded-lg '>
                            <img src={item.cover} alt="harry potter image" className='rounded-md mx-auto h-auto'/>
                            <h1 className='text-lg font-semibold'>{item.title}</h1>
                            <button className='outline bg-red-700 p-2 mt-4 rounded-sm cursor-pointer' onClick={()=>handelmodal(item)}>More Info</button>
                            {showModal &&<Modal 
                              openModal={openModal}
